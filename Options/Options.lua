...
function DF:SetupGUIPages(GUI, CreateCategory, CreateSubTab, BuildPage)
    ...
    BuildPage(pageSorting, function(self, db, Add, AddSpace, AddSyncPoint)
        ...
        local sortByPartyOrder = sortOptionsGroup:AddWidget(GUI:CreateCheckbox(self.child, L["Use Party/Raid Index Order (ignore role)"], db, "sortByPartyOrder", function()
            TriggerSortForCurrentMode()
            UpdateCombatBanner()
        ...
        sortMeleeRanged = sortOptionsGroup:AddWidget(GUI:CreateCheckbox(self.child, L["Separate Melee & Ranged DPS"], db, "sortSeparateMeleeRanged", function()
            TriggerSortForCurrentMode()
            if roleOrderWidget and roleOrderWidget.Refresh then roleOrderWidget.Refresh() end
            UpdateCombatBanner()
        end), 30)
